 ...
 */
class CRM_Smshub_Form_Upload extends CRM_SMS_Form_Upload {
  public $_mailingID;
   ...
   */
  public function setDefaultValues_() {
   ...
   */
  public function buildQuickForm_() {
  ...
  public static function formRule($params, $files, $self) {
    ...
    if (($params['upload_type'] || file_exists($files['textFile']['tmp_name'] ?? '')) ||
      ...
      $str = trim(CRM_Core_TokenSmarty::render(['text' => $str], [
        'smarty' => TRUE,
    ...
    if (
      !empty($params['SMSsaveTemplate']) && in_array(CRM_Utils_Array::value('SMSsaveTemplateName', $params), $templateName)
    ) {
  ...
  public static function formRule2($params, $files, $self) {
    $aa = $params;

    $includeGroups = $self->controller->exportValue('Group', 'includeGroups');
    $excludeGroups = $self->controller->exportValue('Group', 'excludeGroups');
    $includeMailings = $self->controller->exportValue('Group', 'includeMailings');
    $excludeMailings = $self->controller->exportValue('Group', 'excludeMailings');
    $smsName = $self->controller->exportValue('Group', 'name');
    $sms_provider_id = $self->controller->exportValue('Group', 'sms_provider_id');

    $sms_content = $self->controller->exportValue('Upload', 'sms_text_message');

    $include = array_unique(array_merge($includeGroups, $includeMailings));
    $exclude = array_unique(array_merge($excludeGroups, $excludeMailings));

    $groups = array_diff($include, $exclude);

    $groupContacts = \Civi\Api4\GroupContact::get(false)
      ->addSelect('contact_id', 'contact_id.phone_primary')
      ->addWhere('group_id', 'IN', $groups)
      ->execute();
    $curl_script = '';

    $providers = \Civi\Api4\Provider::get(false)
      ->addWhere('id', '=', $sms_provider_id)
      ->execute();
    if ($providers->count() != 1) {
      // Error
      \Drupal::logger('smshub')->error('SMS provider (' . $sms_provider_id . '): Nombre de providers ' . $providers->count());
    ...
    foreach ($groupContacts as $groupContact) {
      ...
      $mailing->id = $self->_mailingID;
      $mess = "body_text";
      $mailing->$mess = $sms_content;

      $sms = CRM_Core_TokenSmarty::render(['text' => $sms_content], [
        'smarty' => true,
        'contactId' => $groupContact['contact_id'],
      ])['text'];

      $phones = \Civi\Api4\Phone::get(false)
        ->addSelect('contact_id', 'phone_numeric', 'is_primary', 'phone_type_id:name')
        ->addWhere('contact_id', '=', $groupContact['contact_id'])
        ->addOrderBy('is_primary', 'DESC')
        ->execute();

      foreach ($phones as $phone) {
        if ((str_starts_with($phone['phone_numeric'], '06')) || (str_starts_with($phone['phone_numeric'], '07')))

          $curl_script .= 'curl -X POST -u ' . $providers->first()['username'] . ':' . $providers->first()['password'] .
            ' -H "Content-Type: application/json" -d \'' .
            json_encode([
              'message' => trim($sms),
              'phoneNumbers' => [$phone['phone_numeric']],
            ], JSON_UNESCAPED_SLASHES) . '\' http://' . $providers->first()['api_url'] . "/message\r\n";

        break;
      }
    }

    $filestr = "filename=\"{$smsName}.script\"";
    CRM_Utils_System::setHttpHeader('Content-Type', 'application/octet-stream');
    CRM_Utils_System::setHttpHeader('Expires', gmdate('D, d M Y H:i:s') . ' GMT');
    CRM_Utils_System::setHttpHeader("Content-Disposition", "inline; $filestr");
    echo $curl_script;
